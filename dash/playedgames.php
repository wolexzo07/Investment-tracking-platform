...
<h4 class="walletFunder"><i class="fa fa-calendar"></i>&nbsp;&nbsp; TRACK <font class="colorg">INVESTMENTS</font>
</h4>

<?php
 $userme = x_clean($_SESSION["IQGAMES_ID_2018_VISION"]);
if(x_count("investment_master","user_id='$userme' LIMIT 1") > 0){
	$counter = 0;
	?>
	<div style="margin-top:20pt;"  class="panel-group" id="accordion">
	<?php
	foreach(x_select("0","investment_master","user_id='$userme'","25","id desc") as $key)
		{
		$counter++;
		$id = $key["id"];
		$tid = $key["tranx_id"];
	...
	x_print("<p class='text-center'><i class='fa fa-calendar hipe'></i><br/></br>No investment record found<br/><br/></p>");
	}
