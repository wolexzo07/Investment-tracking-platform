...
$salt = "ABCDEFGHIJKKLMNOPQ1234567890abcghdtuwioalkjsnh?@";
 ...
 $os = xos();$br = xbr();$ip = xip();
	//	if($create){
if(x_count("userdb_bank","email='$email' AND status='0' OR mobile='$email' AND status='0' LIMIT 1") > 0){
	echo "<p class='hubmsg'>Account Inactive.Please confirm your email!</p>";
	exit();
	}
if(x_count("userdb_bank","email='$email' AND status='1' OR mobile='$email' AND status='1' LIMIT 1") > 0){
...
foreach(x_select("0","userdb_bank","email='$email' AND status='1' OR mobile='$email' AND status='1'","1","name") as $key){
	$id = $key["id"];
...
//echo "<p class='hubmsg'>Failed to login! Try again.</p>";
	...
	finish("ApplicationLogin","Failed to login! Try again");

/**}else{
		//echo "<p class='hubmsg'>Invalid captcha!</p>";
    finish("ApplicationLogin","Invalid captcha!");
	}**/
