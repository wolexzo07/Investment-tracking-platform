...
$salt = "ABCDEFGHIJKKLMNOPQ1234567890abcghdtuwioalkjsnh?@";
 ...
 $tok = "abo_".sha1(uniqid().xrands(10).$email.Date("His"));

 if(empty($check)){
			  //echo "<p class='hubmsg'>You must agree to our terms.</p>";
				finish("ApplicationForm","You must agree to our terms.");
			  exit();
			  }
			  //checking for valid mobile number
			   if(!is_numeric($mobile) || (strlen($mobile) != 11)){
			  //echo "<p class='hubmsg'>You must provide valid phone number.</p>";
					finish("ApplicationForm","You must provide valid phone number.");
			  exit();
			  }
...
//x_insert("plan,status,name,email,pass,token,timest,realtime,os,br,ip,mobile,ref","userdb_bank","'$plan','0','$fullname','$email','$hash','$tok','$time','$rtime','$os','$br','$ip','$mobile','$ref'","<p class='hubmsg'>Account created. <a href='ApplicationLogin'>Login here</a></p>","<p class='hubmsg'>Failed to insert data</p>");

x_insert("plan,status,name,email,pass,token,timest,realtime,os,br,ip,mobile,ref","userdb_bank","'$plan','0','$fullname','$email','$hash','$tok','$time','$rtime','$os','$br','$ip','$mobile','$ref'","<script type='text/javascript'>alert('Account Created successfully!');window.location='ApplicationLogin';</script>","<p class='hubmsg'>Failed to insert data</p>");
}else{
//echo "<font style='color:white;'>".xpost("http://pmcsms.com/api/v1/http.php",$params)."</font>";
//x_insert("plan,status,name,email,pass,token,timest,realtime,os,br,ip,mobile,ref","userdb_bank","'$plan','1','$fullname','$email','$hash','$tok','$time','$rtime','$os','$br','$ip','$mobile','$ref'","<p class='hubmsg'>Account created. <a href='ApplicationLogin'>Login here</a></p>","<p class='hubmsg'>Failed to insert data</p>");

x_insert("plan,status,name,email,pass,token,timest,realtime,os,br,ip,mobile,ref","userdb_bank","'$plan','1','$fullname','$email','$hash','$tok','$time','$rtime','$os','$br','$ip','$mobile','$ref'","<script type='text/javascript'>alert('Account Created successfully!');window.location='ApplicationLogin';</script>","<p class='hubmsg'>Failed to insert data</p>");
}
...
finish("ApplicationForm","Invalid Captcha!Try again.");
 }
